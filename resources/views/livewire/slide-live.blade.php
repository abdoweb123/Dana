...
<div>
    @if (auth('admin')->user()->hasPermissionTo('add slides'))
        <button type="button" class="btn btn-dark my-3 rounded-3" data-bs-toggle="modal"
            data-bs-target="#addModal">
            {{ __('dash.create_new') }}
        </button>
    @endif
    <x-table :columns="['image', 'paragraph_ar', 'paragraph_en', 'group']">
        @forelse ($slides as $slide)
            <tr Role="row" class="odd">
                ...
                <td>
                    @if ($slide->group)
                    {{ $slide->group['title_'.lang()] }}
                    @endif
                </td>
                <td>
                    @if (auth('admin')->user()->hasPermissionTo('edit slides'))
                    ...
                    @endif
                    @if (auth('admin')->user()->hasPermissionTo('delete slides'))
    ...
    <x-modal wireAction="store" id="addModal" title="{{ __('dash.add') }}" type="primary">
        <div class="row">
            ...
            <x-form.select name="group_id" label="{{ __('dash.group') }}">
                @foreach ($groups ?? [] as $g)
                    <x-form.option value="{{ $g->id }}" name="{{ $g['title_'.lang()] }}"></x-form.option>
                @endforeach
            </x-form.select>
